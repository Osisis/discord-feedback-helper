...
client.on(Events.InteractionCreate, async (interaction) => {
  try {
    ...
    // Modal submit → post to suggestions channel + add emoji reactions for voting (starting at 0)
    if (interaction.isModalSubmit() && interaction.customId.startsWith('fb_modal:')) {
      const anon = interaction.customId.endsWith(':1');
      const text = interaction.fields.getTextInputValue('text')?.trim();
      if (!text) {
      ...
      try {
        ...
        // Remove the bot's own reactions so counts start at 0
        // Try the cache first; if missing, refetch the message then remove
        const removeOwn = async (emoji) => {
          try {
            ...
            const r2 = fresh.reactions.cache.get(emoji);
            if (r2) await r2.users.remove(client.user.id);
          } catch (err) {
